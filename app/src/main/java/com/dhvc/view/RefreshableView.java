...
import com.dhvc.R;
import com.dhvc.utils.DensityUtil;
...
public class RefreshableView extends LinearLayout {
    ...
    public RefreshableView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...
//
    ...
    private void initView() {
        ...
        image = mHeaderView.findViewById(R.id.image);
...
//        Log.e(TAG, "onTouchEvent: action = " + action);
    ...
    private void pullScroll() {
        ...
        if (Math.abs(scrollValue) > mRefreshHeight*2
                && mState == State.DRAGGING) {
            // 约定：如果偏移量超过 200(这个值，表示是否可以启动刷新的临界值，可任意定),
            // 那么状态变成 State.READY
            setState(State.LOADTHE);
        }
